...
//    }
    ...
    private void setFrame(int frame, boolean unschedule, boolean animate) {
        if (frame >= mAnimRes.size()) {
            return;
        }
        mAnimating = animate;
        mCurFrame = frame;

        List<Integer> res = mAnimRes.get(frame);
        BitmapDrawable drawable = mImageFetcher.getImageCache(res.get(0));
        if (drawable == null) {
            return;
        }
        mAnimationState.addFrame(drawable, res.get(1));

        selectDrawable(frame);

        int nextFrame = frame + 1;
        if (nextFrame >= mAnimRes.size()) {
            nextFrame = 0;
        }
        mImageFetcher.addImageCache(mAnimRes.get(nextFrame).get(0));
...
//        List<Integer> res = mAnimRes.get(0);
//        Drawable dr = ContextCompat.getDrawable(context, res.get(0));
//        addFrame(dr, res.get(1));

        for (int i = 0; i < 2 && i < mAnimRes.size(); i++) {
            resIds.add(mAnimRes.get(i).get(0));
        }
        if (mImageFetcher == null) {
            mImageFetcher = new ImageFetcher(context.getResources());
        }
        mImageFetcher.initDrawable(resIds);
    }
    ...
    private final static class AnimationState extends DrawableContainerState {
        ...
        public void growArray(int oldSize, int newSize) {
            ...
            System.arraycopy(mDrawables, 0, newDurations, 0, oldSize);
